...
load("WIWA-main-carryover-variables.Rda")
...
ProbAFile <- "./geneland_outputs/GL-Run-Oct21/GL-run-sub-10/proba.pop.membership.txt"
# and tell it how to permute the colors, specific to this data set, too:
...
# set to true if you want to reproject the base-map
...
# now that we have tricked it into resetting the plot are (is that a bug in raster?) 
...
# here is another crazy thought: I could turn the distruct plot into a super high-res raster, and then 
# put it on a latitude and project it and then draw lines from various places on it to the actual
# sampling locations.